...
def encode(
    ...
    if process == "transform":
        ...
        mid = torch.tensor([sum(b) / 2 for b in state_bounds], device=DEVICE, dtype=torch.float)
...
def make_theta_map(theta):
    fig, ax = plt.subplots()

    ax.plot(range(len(theta)), theta)
...
if __name__ == "__main__":
    ...
    for i_episode in range(config["training"]["episodes"]):
        ...
        for t in count():
            ...
            if abs(state[2]) > abs(max_theta):
                max_theta = state[2]
            for i in range(n_actions):
                value_map[i].append((state[2], decode(q_values_enc)[0, i].item()))
            policy_map.append((state[2], action.item()))
            theta_map.append(state[2])
                ...
                wandb.log(
                    {
                        ...
                        "AltitudeMap": make_theta_map(theta_map),
                    }
                )
                break
